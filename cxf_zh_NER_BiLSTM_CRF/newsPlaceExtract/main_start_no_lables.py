...
def result_evaluate(news):
    ...
    f1_p = (1 + beta * beta) * precision_p * recall_p / (beta * beta * precision_p + recall_p)

    precision_p_c = tp_p_c / cnt_predict_p
    recall_p_c = tp_p_c / cnt_samples
    f1_p_c = (1 + beta * beta) * precision_p_c * recall_p_c / (beta * beta * precision_p_c + recall_p_c)
...
# edit by zengzenghe in 20190521 --begin---function：记录预测输出的所有结果
def record_predict_result(new, lst):
    lst.append('id\t' + new.id)
    lst.append('token\t' + new.token)
    lst.append('lables\t' + new.labels)
    lst.append('nerloc\t' + ' '.join(new.nerloc))
    ...
    lst.append('predict_place\tnation(国家):null\tprovince(省):' + new.predict_place['province']
               ...
               + '\tcounty(县):' + new.predict_place['county'])
    lst.append('\n')
# edit by zengzenghe in 20190521 --end---function：记录预测输出的所有结果



def main():
    # inpurt_file = 'conf/merge_before.txt'
    merge_file = 'input/News_weibo_merge.txt'
    news = parse_merge_file(merge_file)
    lst = []
    for new in news:
        # 方法2：运用打分制
        extract_place(new)
        record_predict_result(new, lst)
    with open('result/News_weibo_result.txt', 'w', encoding = 'utf-8') as f:
        f.write('\n'.join(lst))
