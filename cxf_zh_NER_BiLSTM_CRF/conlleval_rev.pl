...
my $FB05 = 0.0;            # FB05 score (Van Rijsbergen 1979)
...
if (not $latex) {
   # compute overall precision, recall and FB05 (default values are 0.0)
   $precision = 100*$correctChunk/$foundGuessed if ($foundGuessed > 0);
   $recall = 100*$correctChunk/$foundCorrect if ($foundCorrect > 0);
   $FB05 = (1+0.5*0.5)*$precision*$recall/(0.5*0.5*$precision+$recall)
...
if (not $latex) {
   for $i (@sortedTypes) {
      ...
      if ($precision+$recall == 0.0) { $FB05 = 0.0; }
      else { $FB05 = (1+0.5*0.5)*$precision*$recall/(0.5*0.5*$precision+$recall); }
      printf "%17s: ",$i;
      printf "precision: %6.2f%%; ",$precision;
      printf "recall: %6.2f%%; ",$recall;
      printf "FB05: %6.2f  %d\n",$FB05,$foundGuessed{$i};
